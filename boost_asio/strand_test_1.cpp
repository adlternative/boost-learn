...
void WorkerThread(boost::shared_ptr<boost::asio::io_service> iosvc,
  ...
  std::cout << "Thread " << boost::this_thread::get_id() << ", " << counter
            << " Start.\n";
...
void async_send_handler(int number) {
  std::cout << "Number: " << number
            << ", threadID: " << boost::this_thread::get_id() << std::endl;
}
...
int main(void) {
  ...
  // and you want the handlers of this async_send call to be invoked
  // sequentially
