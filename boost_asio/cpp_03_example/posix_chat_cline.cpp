...
public:
  posix_chat_client(boost::asio::io_context &io_context,
      ...
      : socket_(io_context), input_(io_context, ::dup(STDIN_FILENO)),
        output_(io_context, ::dup(STDOUT_FILENO)),
        input_buffer_(chat_message::max_body_length) {
    boost::asio::async_connect(socket_, endpoints,
                               boost::bind(&posix_chat_client::handle_connect,
                                           this,
...
private:
  void handle_connect(const boost::system::error_code &error) {
    if (!error) {
      ...
      /* 异步读标准输出读到换行符停下来执行 handle_read_input */
  ...
  void handle_read_input(const boost::system::error_code &error,
    ...
    if (!error) {
      ...
      /* 从input_buffer_中拿走length-1字节（不包含换行）添加到write_msg
       似乎input_buffer_的内容也会减少 */
